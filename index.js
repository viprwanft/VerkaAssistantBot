...
LINKS:
- Platform (use this link always): https://app.rwanftfi.com/?ref=ProCripto
...
bot.on("message", async (msg) => {
  ...
  if (msg.chat.type !== "private") {
    ...
    // Respond if: user already talked to bot OR mentioned OR replied to bot
    const hasHistory = histories.has(userId) && getHistory(userId).length > 0;
    if (!hasHistory && !mentionedBot && !repliedToBot) return;
  }
