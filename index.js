...
// Сет для отслеживания тех, кого Вера уже поприветствовала (чтобы не было дублей)
...
// ПЕРЕХВАТ ИЗМЕНЕНИЙ СТАТУСА (Срабатывает, когда Group Help снимает ограничения)
bot.on("chat_member", async (update) => {
  ...
  // Ловим переход: когда пользователь БЫЛ ограничен капчей (restricted),
  // а теперь стал либо member (чистый), либо его статус restricted обновился (Group Help размутил кнопки)
  const wasRestricted = update.old_chat_member && update.old_chat_member.status === "restricted";
  const isOkNow = member.status === "member" || (member.status === "restricted" && member.can_send_messages === true);

  if ((wasRestricted && isOkNow) || (member.status === "member" && (!update.old_chat_member || update.old_chat_member.status === "left"))) {
    
    // Защита от дублей: если уже отправляли этому юзеру — выходим
    ...
    // Отправка ОДНИМ сообщением (Текст зашит внутрь Caption под видео)
    if (welcomeVideos[lang]) {
      try {
        await bot.sendVideo(chatId, welcomeVideos[lang], { caption: rawText, parse_mode: "Markdown" });
      } catch (err) {
        console.error(`Error sending welcome video:`, err.message);
...
// Обработчик текста и видео от админа
bot.on("message", async (msg) => {
  ...
  if (reply) {
    bot.sendMessage(msg.chat.id, reply, { reply_to_message_id: msg.message_id, parse_mode: "Markdown" });
  }
});
